"""This module represents the main module of the project."""
# ################################################################
# ########   SaveYourFridge
# ################################################################
...
# Helper function to count the messages sent for debugging purposes
def countMessages():
    """ Read the last count of messages already sent from the NVRAM and adds
    1 whenever it is called. This function is used for statistics and debugging
    purposes """
...
def console(message):
    """ Implementation of a simple logging functionself.

    Args:
        message (str): The message which should be logged."""
    if low_power_consumption_mode == 0:
        now = time.localtime()
        print("%s-%02d-%02d %02d:%02d:%02d [%s] %s" % (now[0],now[1],now[2],now[3],now[4],now[5], device_id, message))

# Helper function for reading key/value pairs stored in NVRAM
def nvram_read(key):
    """ Helper function for reading NVRAM values.

    Args:
        key (str): The key for the value which should be retrieved from NVRAM
    Returns:
        ret (int): The value for the corresponding key read from NVRAM"""
    ret=pycom.nvs_get(key)
    return ret

# Helper function for writing key/value pairs to NVRAM
def nvram_write(key,value):
    """ Helper function for writing NVRAM values.

    Args:
        key   (str): The key for the corresponding value which should be written
        to the NVRAM.
        value (int): The value which should be written to NVRAM.
    """
    pycom.nvs_set(key, int(value))

# Controlling the status led
def led_blink(color,duration,loop=1):
    """ Function for controlling the status LED.

    Args:
        color    (str): String for the LED Color in hex format
        duration (int): Duration of the time between the loops in ms
        loop     (int): Amount of LED "blinks"
    """
    ...
    if loop > 0:
        ...
        while True:
            ...
            pycom.rgbled(color_black)
            time.sleep(duration)
# Function for signal error LED light
# TODO: Removing this Function and use led_blink
def led_signal_error():
    """ Function to use the LED on signal erro."""
    led_blink(color_red, 0.2, 0)

# Sending function for sigfox network
def send_sigfox_message(message):
    """ Sending a message through the Sigfox Network.

    Args:
        message (str): The payload which should be send. Limit 12 Bytes!
    """
    try:
